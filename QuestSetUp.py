...
def create_all_quests():
    ...
    ariane.add_quest(rune_mysteries, 1)
    ariane.add_quest(rune_memories, 2)
    ariane.add_quest(heart_of_stone, 3)

    camelot_series = Camelot_Series()
    camelot_series.add_quest(merlins_crystal, 1)
    camelot_series.add_quest(holy_grail, 2)
    camelot_series.add_quest(kings_ransom, 3)
    camelot_series.add_related_quest(murder_mystery)

    curse_of_arrav = Curse_Of_Arrav()
    curse_of_arrav.add_quest(shield_of_arrav, 1)
    curse_of_arrav.add_quest(defender_of_varrock, 2)
    curse_of_arrav.add_quest(the_curse_of_arrav, 3)
    curse_of_arrav.add_quest(ritual_of_the_mahjarrat, 4)

    desert_series = Desert_Series()
    desert_series.add_quest(stolen_hearts, 1)
    desert_series.add_quest(diamond_in_the_rough, 2)
    desert_series.add_quest(icthlarins_little_helper, 3)
    desert_series.add_quest(spirits_of_the_elid, 4)
    desert_series.add_quest(contact, 5)
    desert_series.add_quest(dealing_with_scabaras, 6)
    desert_series.add_quest(smoking_kills, 7)
    desert_series.add_quest(missing_my_mummy, 8)
    desert_series.add_quest(the_jack_of_spades, 9)
    desert_series.add_quest(crocodile_tears, 10)
    desert_series.add_quest(do_no_evil, 11)
    desert_series.add_quest(our_man_in_the_north, 12)
    desert_series.add_quest(phite_club, 13)

    dorgeshuun_series = Dorgeshuun_Series()
    dorgeshuun_series.add_quest(the_lost_tribe, 1)
    dorgeshuun_series.add_quest(death_to_the_dorgeshuun, 2)
    dorgeshuun_series.add_quest(another_slice_of_ham, 3)
    dorgeshuun_series.add_quest(land_of_the_goblins, 4)
    dorgeshuun_series.add_quest(the_chosen_commander, 5)
    ...
    dragonkin_series.add_quest(a_tail_of_two_cats, 1)
    dragonkin_series.add_quest(while_guthix_sleeps, 2)
    dragonkin_series.add_quest(ritual_of_the_mahjarrat, 3)
    dragonkin_series.add_quest(missing_presumed_death, 4)
    dragonkin_series.add_quest(one_of_a_kind, 5)
    dragonkin_series.add_quest(heros_welcome, 6)
    dragonkin_series.add_quest(sliskes_endgame, 7)

    druids_circles_series = Druids_Circle_Series()
    druids_circles_series.add_quest(druidic_ritual, 1)
    druids_circles_series.add_quest(eadgars_ruse, 2)
    ...
    elder_gods.add_quest(fate_of_the_gods, 1)
    elder_gods.add_quest(heart_of_stone, 2)
    elder_gods.add_quest(children_of_mah, 3)
    elder_gods.add_quest(sliskes_endgame, 4)
    elder_gods.add_related_quest(one_of_a_kind)

    elemental_workshop_series = Elemental_Workshop_Series()
    elemental_workshop_series.add_quest(elemental_workshop_i, 1)
    elemental_workshop_series.add_quest(elemental_workshop_ii, 2)
    elemental_workshop_series.add_quest(elemental_workshop_iii, 3)
    elemental_workshop_series.add_quest(elemental_workshop_iv, 4)

    elf_series = Elf_Series()
    elf_series.add_quest(plague_city, 1)
    elf_series.add_quest(biohazard, 2)
    elf_series.add_quest(underground_pass, 3)
    elf_series.add_quest(regicide, 4)
    elf_series.add_quest(roving_elves, 5)
    elf_series.add_quest(mournings_end_part_i, 6)
    elf_series.add_quest(mournings_end_part_ii, 7)
    elf_series.add_quest(within_the_light, 8)
    elf_series.add_quest(plagues_end, 9)
    elf_series.add_related_quest(sheep_herder)
    elf_series.add_related_quest(waterfall_quest)
    elf_series.add_related_quest(catapult_construction)
    elf_series.add_related_quest(the_prisoner_of_glouphrie)
    elf_series.add_related_quest(the_light_within)

    enchanted_key_series = Enchanted_Key_Series()
    enchanted_key_series.add_quest(making_history, 1)
    enchanted_key_series.add_quest(meeting_history, 2)
    enchanted_key_series.add_quest(the_light_within, 3)

    fairy_tale_series = Fairy_Tale_Series()
    fairy_tale_series.add_quest(lost_city, 1)
    fairy_tale_series.add_quest(fairy_tale_i_growing_pains, 2)
    fairy_tale_series.add_quest(fairy_tale_ii_cure_a_queen, 3)
    fairy_tale_series.add_quest(fairy_tale_iii_battle_at_orks_rift, 4)

    fremennik = Fremennik()
    fremennik.add_quest(the_fremennik_trials, 1)
    fremennik.add_quest(horror_from_the_deep, 2)
    fremennik.add_quest(throne_of_miscellania, 3)
    fremennik.add_quest(royal_trouble, 4)
    fremennik.add_quest(lunar_diplomacy, 5)
    fremennik.add_quest(the_fremennik_isles, 6)
    fremennik.add_quest(dream_mentor, 7)
    fremennik.add_quest(glorious_memories, 8)
    fremennik.add_quest(blood_runs_deep, 9)
    ...
    god_series.add_quest(the_world_wakes, 1)
    god_series.add_quest(the_death_of_chivalry, 2)
    god_series.add_quest(missing_presumed_death, 3)
    god_series.add_quest(fate_of_the_gods, 4)
    god_series.add_quest(the_mighty_fall, 5)
    god_series.add_quest(dishonour_among_thieves, 6)
    god_series.add_quest(heros_welcome, 7)
    god_series.add_quest(the_light_within, 8),
    god_series.add_quest(nomads_elegy, 9)
    god_series.add_quest(children_of_mah, 10)
    god_series.add_quest(sliskes_endgame, 11)

    gnome_series = Gnome_Series()
    gnome_series.add_quest(the_grand_tree, 1)
    gnome_series.add_quest(monkey_madness, 2)
    gnome_series.add_quest(the_eyes_of_glouphrie, 3)
    gnome_series.add_quest(the_path_of_glouphrie, 4)
    gnome_series.add_quest(the_prisoner_of_glouphrie, 5)
    ...
    linza.add_quest(deadliest_catch, 1)
    linza.add_quest(kindred_spirits, 2)

    mahjarrat_quest_series = Mahjarrat_Quest_Series()
    mahjarrat_quest_series.add_quest(the_tale_of_the_muspah, 1)
    mahjarrat_quest_series.add_quest(missing_my_mummy, 2)
    mahjarrat_quest_series.add_quest(the_curse_of_arrav, 3)
    mahjarrat_quest_series.add_quest(the_temple_at_senntisten, 4)
    mahjarrat_quest_series.add_quest(ritual_of_the_mahjarrat, 5)

    monkey_series = Monkey_Series()
    monkey_series.add_quest(monkey_madness, 1)
    monkey_series.add_quest(recipe_for_disaster, 2)
    monkey_series.add_quest(do_no_evil, 3)

    myreque_series = Myreque_Series()
    myreque_series.add_quest(priest_in_peril, 1)
    myreque_series.add_quest(nature_spirit, 2)
    myreque_series.add_quest(in_search_of_the_myreque, 3)
    myreque_series.add_quest(in_aid_of_the_myreque, 4)
    myreque_series.add_quest(darkness_of_hallowvale, 5)
    myreque_series.add_quest(legacy_of_seergaze, 6)
    myreque_series.add_quest(the_branches_of_darkmeyer, 7)
    myreque_series.add_quest(the_lord_of_vampyrium, 8)
    myreque_series.add_quest(river_of_blood, 9)
    myreque_series.add_related_quest(shades_of_mortton)

    penguin_series = Penguin_Series()
    penguin_series.add_quest(cold_war, 1)
    penguin_series.add_quest(hunt_for_red_raktuber, 2)
    penguin_series.add_quest(some_like_it_cold, 3)
    penguin_series.add_quest(back_to_the_freezer, 4)
    penguin_series.add_related_quest(recipe_for_disaster)

    pirate_quest_series = Pirate_Quest_Series()
    pirate_quest_series.add_quest(pirates_treasure, 1)
    pirate_quest_series.add_quest(rum_deal, 2)
    pirate_quest_series.add_quest(cabin_fever, 3)
    pirate_quest_series.add_quest(the_great_brain_robbery, 4)
    pirate_quest_series.add_quest(rocking_out, 5)
    pirate_quest_series.add_quest(a_clockwork_syringe, 6)
    pirate_quest_series.add_quest(pieces_of_hate, 7)

    other_mahjarrat_quests = Other_Mahjarrat_Quests()
    other_mahjarrat_quests.add_quest(fight_arena, 1)
    other_mahjarrat_quests.add_quest(hazeel_cult, 2)
    other_mahjarrat_quests.add_quest(desert_treasure, 3)
    other_mahjarrat_quests.add_quest(enakhras_lament, 4)

    ozan = Ozan_Signature_Heroes_Quests()
    ozan.add_quest(stolen_hearts, 1)
    ...
    sea_slug_series.add_quest(kenniths_concerns, 3)
    sea_slug_series.add_quest(salt_in_the_wound, 4)
    ...
    temple_knight_series.add_quest(the_slug_menace, 3)
    temple_knight_series.add_quest(while_guthix_sleeps, 4)
    temple_knight_series.add_quest(ritual_of_the_mahjarrat, 5)
    temple_knight_series.add_quest(the_death_of_chivalry, 6)
    temple_knight_series.add_related_quest(devious_minds)

    the_raptor = The_Raptor_Signature_Heroes_Quests()
    the_raptor.add_quest(song_from_the_depths, 1)

    the_return_of_zaros = The_Return_Of_Zaros()
    the_return_of_zaros.add_quest(the_dig_site, 1)
    the_return_of_zaros.add_quest(desert_treasure, 2)
    the_return_of_zaros.add_quest(the_temple_at_senntisten, 3)
    the_return_of_zaros.add_quest(the_world_wakes, 4)
    the_return_of_zaros.add_quest(fate_of_the_gods, 5)
    the_return_of_zaros.add_related_quest(devious_minds)
    the_return_of_zaros.add_related_quest(the_firemakers_curse)

    the_rise_of_lucien = The_Rise_Of_Lucien()
    the_rise_of_lucien.add_quest(temple_of_ikov, 1)
    ...
    void_knights_series.add_quest(the_void_stares_back, 30)
    void_knights_series.add_related_quest(imp_catcher)

    xenia = Xenia_Signature_Heroes_Quests()
    xenia.add_quest(the_blood_pact, 1)
    xenia.add_quest(carnillean_rising, 2)
